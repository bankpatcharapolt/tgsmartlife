...
<div class="col-md-12 col-sm-12  ">
    <div class="x_panel">
        <div class="x_title">
            <h2>แก้ไข สินค้า <small></small></h2>
...
<script>
    ...
    function drawtProductTag(base_url){
        ...
        if(results.datas.length > 0){ 
            var ptag = results.datas[0].tag;
            if( ptag != '' && ptag != null){
                arr_tag = ptag.split(",");
            }

            if(results.datas[0].sub_category_id){
               
                $("#sub_category_id").val(results.datas[0].sub_category_id);
                $('#sub_category_id').trigger('change');

            }
        }

        var product_tag = get_product_tag_used(base_url);
        if(product_tag.datas.length > 0){ 
            var tags = '';
            $.each( product_tag.datas, function( key, val ) {
                var checked = '';
                if(jQuery.inArray(val.id, arr_tag) !== -1){
                    checked = 'checked';
                }
                console.log('value="'+val.id+' = '+checked);
                tags += '<div class="form-check-inline">';
                tags += '<label class="form-check-label">';
                tags += '<input type="checkbox" class="form-check-input" name="tag[]" value="'+val.id+'"  '+checked+'>'+val.name+'';
                tags += '</label>';
                tags += '</div>';
            });
    ...
    function drawform(base_url){
        ...
        if(results.datas.length > 0){
            ...
            if(results.datas[0].manual_path != '' && results.datas[0].manual_path != null){
                ...
                str += '<button style="position: absolute; right: 0px; color: white; background-color: #ff9500; border: 1px solid #ff9500; border-radius: 3px; right: 8px;top: 2px;" onclick="delPDFManual('+results.datas[0].id+')">X</button>';
    ...
    function get_images(base_url){
        var res = null;
        $.ajax({
            url: base_url+'/admin/Product/get_images', //ทำงานกับไฟล์นี้
            data: {
                "id" : $('form#action-form #id').val()
            },  //ส่งตัวแปร
            type: "POST",
    ...
    function delImage(id, ref_id){
        $.ajax({
            url: base_url+'/admin/Product/del_image', //ทำงานกับไฟล์นี้
            data: {
                "id" : id,
                "ref_id" : ref_id
            },  //ส่งตัวแปร
    ...
    $("form#action-form").on("submit",function(e){ // จะทำงานก็ต่อเมื่อกด submit ฟอร์ม
        ...
        //### Files Images ###//
        if($('form#action-form #images')[0].files.length > 0 ){
            var inputFiles = $('form#action-form #images').files; //เป็นการดึงข้อมูลรูปภาพเพื่อเตรียมเช็คไฟล์ก่อนทำงานส่วน Ajax
            fd.append('file',inputFiles); //ใช้ในการแทรกค่าไฟล์รูปภาพใน element
        }
    ...
    function delPDFManual(id){
        $.ajax({
            url: base_url+'/admin/Product/del_product_manual', //ทำงานกับไฟล์นี้
            data: {
                "id" : id,
