    ...
    <div class="x_panel">
        <div class="x_title">
            ...
            <!-- <ul class="nav navbar-right panel_toolbox">
                <li>
                    <a href="<?=base_url('admin_product_spec_add');?>" class="btn add-product-cate" target="_blank"  style="color: #466889;"><i class="fa fa-plus"></i> ข้อมูลผลิตภันฑ์</a>
                </li>
            </ul> -->
            <div class="clearfix"></div>
        </div>
            ...
            <div class="table-responsive">
                <table id="table" class="table table-striped jambo_table bulk_action" style="width:100%; border-spacing: 1px !important;">
                    ...
                    <tr class="headings">
                        ...
                        <th class="column-title" style="width: 4%;">หมวดหมู่ </th>
...
<script>
    ...
    function drawtable(base_url){
        ...
        if(results.datas.length > 0){   
            ...
            $.each( results.datas, function( key, val ) {
                ...
                tr += '<li><a href="'+base_url+'admin_product_spec_action/'+val.id+'" class="btn btn-round btn-warning" style="font-size: 11px; padding: 4px 8px;" data-toggle="tooltip" title="แก้ไข"><i class="fa fa-wrench"></i></a></li>';
    ...
    function get_results(base_url){
        var res = null;
        $.ajax({
            url: base_url+'admin/Product/get_product_spec', //ทำงานกับไฟล์นี้
            data: '',  //ส่งตัวแปร
            type: "POST",
            dataType: 'json',
            async:false,
    ...
    function delResult($product_id){
        if(confirm("คุณต้องการที่จะลบข้อมูลสินค้านี้จริงหรือไม่ ")){
            var res = null;
            $.ajax({
                url: base_url+'/admin/Product/del_spec', //ทำงานกับไฟล์นี้
                data: {
