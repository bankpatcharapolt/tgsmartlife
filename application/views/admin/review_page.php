...
<script>
    ...
    function drawtable(base_url){
        ...
        if(results.datas.length > 0){   
            ...
            $.each( results.datas, function( key, val ) {
                ...
                tr += '<li><a href="'+base_url+'admin_review_edit/'+val.id+'" class="btn btn-round btn-warning" style="font-size: 11px; padding: 4px 8px;" data-toggle="tooltip" title="แก้ไข"><i class="fa fa-wrench"></i></a></li>';
    ...
    function get_results(base_url){
        var res = null;
        $.ajax({
            url: base_url+'admin/Review/get_results', //ทำงานกับไฟล์นี้
            data: '',  //ส่งตัวแปร
            type: "POST",
            dataType: 'json',
            async:false,
    ...
    function delImage($id){
        if(confirm("คุณต้องการที่จะลบ ข้อมูลนี้ จริงหรือไม่ ? ")){
            var res = null;
            $.ajax({
                url: base_url+'/admin/Review/del_result', //ทำงานกับไฟล์นี้
                data: {
                    "id" : $id
                },  //ส่งตัวแปร
                type: "POST",
