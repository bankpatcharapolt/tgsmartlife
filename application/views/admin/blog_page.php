...
<div class="col-md-12 col-sm-12  ">
    <div class="x_panel">
        <div class="x_title">
            <h2>Knowledge<small></small></h2>
            <input name="base_url" value="<?=base_url();?>" type="hidden" >
            <ul class="nav navbar-right panel_toolbox">
                <li>
                    <a href="<?=base_url('admin_blog_add');?>" class="btn add-blog" target="_blank"  style="color: #466889;"><i class="fa fa-plus"></i> Knowledge</a>
                </li>
            </ul>
            <div class="clearfix"></div>
        </div>
            ...
            <div class="table-responsive">
                <table id="table" class="table table-striped jambo_table bulk_action" style="width:100%; border-spacing: 1px !important;">
                    ...
                    <tr class="headings">
                        ...
                        <th class="column-title" style="width: 11%;">หัวข้อย่อย</th>
...
<script>
    ...
    function drawtable(base_url){
        ...
        if(results.datas.length > 0){   
            ...
            $.each( results.datas, function( key, val ) {
                ...
                tr += '<td style="vertical-align: inherit;">'+val.sub_header+'</td>';

                var product_cate = (val.product_cate == '99999')?'อื่นๆ':val.category_name;
                tr += '<td style="vertical-align: inherit;">'+product_cate+'</td>';
                ...
                tr += '<li><a href="'+base_url+'admin_blog_edit/'+val.id+'" class="btn btn-round btn-warning" style="font-size: 11px; padding: 4px 8px;" data-toggle="tooltip" title="แก้ไข"><i class="fa fa-wrench"></i></a></li>';
                tr += '<li><button class="btn btn-round btn-danger" style="font-size: 11px; padding: 4px 8px;" data-toggle="tooltip" title="ลบข้อมูล" onclick="delImage('+val.id+')"><i class="fa fa-trash"></i></button></li>';
                tr += '</ul>';
                tr += '</td>';
    ...
    function get_results(base_url){
        var res = null;
        $.ajax({
            url: base_url+'admin/Blog/get_results', //ทำงานกับไฟล์นี้
            data: '',  //ส่งตัวแปร
            type: "POST",
            dataType: 'json',
            async:false,
    ...
    function delImage($id){
        if(confirm("คุณต้องการที่จะลบ ข้อมูลนี้ จริงหรือไม่ ? ")){
            var res = null;
            $.ajax({
                url: base_url+'/admin/Blog/del_result', //ทำงานกับไฟล์นี้
                data: {
                    "id" : $id
                },  //ส่งตัวแปร
                type: "POST",
