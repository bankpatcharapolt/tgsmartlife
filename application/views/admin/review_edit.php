                ...
                <div class="col-md-12">
                    ...
                    <div class="form-group row">
                        <div class="col-md-12 ">
                            ...
                            <textarea class="form-control" rows="5" id="sub_header" name="sub_header"></textarea>
                        </div>
                    </div>
                    <div class="form-group row">
                        <div class="col-sm-12">
                    ...
                    <div class="form-group" style="border-top: 1px solid #dee2e6;">
                        <div class="col-12" style="padding: 13px 0;">
                            <a id="back-btn" href="<?=base_url("admin_review");?>" class="btn btn-warning text-center" style="" type="button">กลับ</a>
...
<script>
    ...
    function get_seo_used(base_url){
        var res = null;
        $.ajax({
            url: base_url+'admin/Review/get_review_seo_results', //ทำงานกับไฟล์นี้
            data: '',  //ส่งตัวแปร
            type: "POST",
            dataType: 'json',
            async:false,
    ...
    function get_results(base_url){
        var res = null;
        $.ajax({
            url: base_url+'/admin/Review/get_results_once', //ทำงานกับไฟล์นี้
            data: {
    ...
    function set_image(base_url){
        ...
        images.datas.forEach(element => {
            ...
            str += '<button style="position: absolute; right: 0px; color: white; background-color: #ff9500; border: 1px solid #ff9500; border-radius: 3px; right: 8px;top: 2px;" onclick="delImage('+element.id+','+element.ref_id+',\''+element.group+'\')">X</button>';
    ...
    function get_images(base_url){
        var res = null;
        $.ajax({
            url: base_url+'/admin/Review/get_images', //ทำงานกับไฟล์นี้
            data: {
                "id" : $('form#edit-form input[name="id"]').val()
            },  //ส่งตัวแปร
            type: "POST",
    ...
    function delImage(id, ref_id, group){
        $.ajax({
            url: base_url+'/admin/Review/del_image', //ทำงานกับไฟล์นี้
            data: {
                "id" : id,
                "ref_id" : ref_id,
                "group" : group
    ...
    $("form#edit-form").on("submit",function(e){ // จะทำงานก็ต่อเมื่อกด submit ฟอร์ม
        ...
        $.ajax({
            url:base_url+'admin/Review/review_actions', //ให้ระบุชื่อไฟล์ PHP ที่เราจะส่งค่าไป
            type:'post',
            dataType: 'json',
