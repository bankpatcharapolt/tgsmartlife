...
<script>
    ...
    function drawform(base_url){
        ...
        if(results){
            $('form#action-form #reg_name').val(results.datas[0].reg_name);
            $('form#action-form #reg_address').val(results.datas[0].reg_address);
            $('form#action-form #reg_telephone').val(results.datas[0].reg_telephone);
            $('form#action-form #reg_email').val(results.datas[0].reg_email);
            $('form#action-form #latitude').val(results.datas[0].latitude);
            $('form#action-form #longitude').val(results.datas[0].longitude);
    ...
    function get_results(base_url){
        var res = null;
        $.ajax({
            url: base_url+'/admin/Helpservice/get_results_once', //ทำงานกับไฟล์นี้
            data: {
