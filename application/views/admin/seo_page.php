    ...
    <div class="x_panel">
        <div class="x_title">
            ...
            <ul class="nav navbar-right panel_toolbox">
                <li>
                    <a href="<?=base_url('admin_seo_add');?>" class="btn add-blog" style="color: #466889;"><i class="fa fa-plus"></i> SEO</a>
                </li>
            </ul>
            <div class="clearfix"></div>
        </div>
            ...
            <div class="table-responsive">
                <table id="table" class="table table-striped jambo_table bulk_action" style="width:100%; border-spacing: 1px !important;">
                    ...
                    <tr class="headings">
                        ...
                        <th class="column-title" style="width: 10%;">Page</th>
                        <th class="column-title" style="width: 10%;">Seo Title</th>
                        <th class="column-title" style="width: 11%;">Seo Keyword </th>
                        <th class="column-title" style="width: 11%;">Category </th>
...
<script>
    ...
    function drawtable(base_url){
        ...
        if(results.datas.length > 0){   
            ...
            $.each( results.datas, function( key, val ) {
                ...
                tr += '<td style="vertical-align: inherit;">'+val.page+'</td>';
                tr += '<td style="vertical-align: inherit;">'+val.seo_title+'</td>';
                tr += '<td style="vertical-align: inherit; text-align: center;">'+val.seo_keyword+'</td>';
                tr += '<td style="vertical-align: inherit; text-align: center;">'+val.category+'</td>';
                ...
                tr += '<li><a href="'+base_url+'admin_seo_edit/'+val.id+'" class="btn btn-round btn-warning" style="font-size: 11px; padding: 4px 8px;" data-toggle="tooltip" title="แก้ไข"><i class="fa fa-wrench"></i></a></li>';
    ...
    function get_results(base_url){
        var res = null;
        $.ajax({
            url: base_url+'admin/Seo/get_results', //ทำงานกับไฟล์นี้
            data: '',  //ส่งตัวแปร
            type: "POST",
            dataType: 'json',
            async:false,
    ...
    function del_results($id){
        if(confirm("คุณต้องการที่จะลบ ข้อมูลนี้ จริงหรือไม่ ? ")){
            var res = null;
            $.ajax({
                url: base_url+'/admin/Seo/del_result', //ทำงานกับไฟล์นี้
                data: {
                    "id" : $id
                },  //ส่งตัวแปร
                type: "POST",
