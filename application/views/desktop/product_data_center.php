	...
	<script>
		...
		drawcontens(base_url, rowno);
		function drawspecdetail(product_id = null){
			var base_url = "<?= base_url(); ?>";
			var results = get_result_specdetail(base_url, product_id);
			var html = "";
			$.each(results.datas, function(key, val) {
				console.log(val);
				if (val.detail != '' && val.detail != null) {
						var product_spec = val.detail;
						$("#spec_detail_" + val.product_id).html(product_spec);
				}
			});
		}
		function drawcontens(base_url, rowno = null) {
			//###  ressults ###//
			var results = get_results(base_url, rowno);
			console.log(results);
			if (results.datas.length > 0) {
				var product = '';
				var product_spec = '';
				$.each(results.datas, function(key, val) {
					product += '<div class="col-lg-3" onclick="drawspecdetail('+val.id+')">';
					...
					product_spec += '<div id="spec_detail_'+val.id+'"></div>';
					if (val.spec_warranty != '' && val.spec_warranty != null) {
						product_spec += '<p><a href="'+base_url+'warranty/'+val.id+'" target="_blank">ข้อมูลการรับประกัน ' + val.name + '</a></p>';
					}
					product_spec += '</div>';
					product_spec += '</div>';
		...
		function get_results(base_url, rowno = null) {
			var res = null;
			$.ajax({
				url: base_url + 'Main/get_product_data_center_results/' + rowno, //ทำงานกับไฟล์นี้
				data: {rowno : rowno}, //ส่งตัวแปร
				type: "POST",
				dataType: 'json',
				async: false,
		...
		function get_result_specdetail(base_url, id = null) {
			var res = null;
			$.ajax({
				url: base_url + 'Main/get_product_detail_results', //ทำงานกับไฟล์นี้
