...
class Main extends CI_Controller {
	...
	public function get_product_data_center_results() {  
		//$cate_id = $this->input->post('cate_id');
	...
	public function get_review_results_paginate() {  
	 ...
	 public function get_products_count($cate_id = null) {
		...
		WHERE `product_category`.`active` = 1  AND `product`.`active` = 1 ";
		if(!empty($cate_id)){
			$Query .= " AND `product`.`category` = ".$cate_id ;
		}
	...
	public function get_products_results($row_number = null) {
		...
		$config['total_rows'] = $this->get_products_count($category_id);
		$config['per_page'] = 9;
		$config["uri_segment"] = 3;
	...
	public function get_product_related() {  
		$tag = $this->input->post('tag');
        $Query = "SELECT `product`.* FROM `product` WHERE `product`.`active` = 1 AND `product`.`tag` LIKE '%".$tag."%';";
		$result = $this->db->query($Query)->result_array();
