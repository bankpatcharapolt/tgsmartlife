...
class Review extends Main {
	...
	public function review_page(){	
		...
		$this->load->view('admin/review_page');
	...
	public function get_results() {  
		...
		$Query .=" order by review.created desc,review.updated desc"; // add on 20231021
	...
	public function del_image() { 
		...
		$Query = " SELECT * FROM images WHERE id = '".$id."' AND ref_id = '".$ref_id."' AND `group` = 'review'";
		$result = $this->db->query($Query)->result();
	...
	public function review_actions(){
        ...
        if($method == 'insert'){
			...
			$insert = $this->db->insert('review', $obj);
            if($insert){
                $id = $this->db->insert_id();
                $res->status = true;
                //$res->datas = base_url('admin/product');
                $res->datas = base_url('admin_review_edit/'.$id);
		...
		//###  UPDATE ###//
        if($method == 'update'){
			// print_r($this->db->mysql_error());exit();
			$obj['updated'] = $curent_date;
            $this->db->where("id", $id);
            $this->db->update("review", $obj); 
	...
	public function do_upload_thumnel($files, $id){
        ...
        ##### create path ####
		$upload_path = "./uploads/images/review/".$id;
	...
	public function do_upload_image($files, $id, $i){
        ...
        ##### create path ####
		$uploads_path = "./uploads/images/review/".$id;
	...
	public function del_result() { 
		...
		//### del files ###///
		$this->load->helper('file');
		delete_files('./uploads/images/review/'.$id.'/', true); // Delete files into the folder
		@rmdir('./uploads/images/review/'.$id.'/'); // Delete the folder
	...
	public function get_review_seo_results() {  
		//$cate_id = $this->input->post('cate_id');
        $Query = "SELECT * FROM page_seo WHERE category = 'review'";
		$result = $this->db->query($Query)->result_array();
